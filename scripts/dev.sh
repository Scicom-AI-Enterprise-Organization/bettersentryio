...
PG_DSN='postgres://bettersentryio:bettersentryio@127.0.0.1:5432/bettersentryio?sslmode=disable'
ENGINE_PORT=9090
WEB_PORT=3100
...
ensure_postgres() {
  if pg_isready -h 127.0.0.1 -p 5432 >/dev/null 2>&1; then
    ...
    pg_isready -h 127.0.0.1 -p 5432 >/dev/null 2>&1 || { fail "postgres did not start"; return 1; }
  fi

  psql -h 127.0.0.1 -p 5432 -d postgres -tAc \
    "select 1 from pg_roles where rolname='bettersentryio'" | grep -q 1 ||
    psql -h 127.0.0.1 -p 5432 -d postgres -qc \
      "create role bettersentryio login password 'bettersentryio'"

  psql -h 127.0.0.1 -p 5432 -d postgres -tAc \
    "select 1 from pg_database where datname='bettersentryio'" | grep -q 1 ||
    createdb -h 127.0.0.1 -p 5432 -O bettersentryio bettersentryio
...
up() {
  ...
  if [ -d web/node_modules ]; then
    if [ -n "$(port_pid $WEB_PORT)" ]; then
      ...
      say "starting UI on :$WEB_PORT"
      ( cd web && set -a && . ./.env && set +a && \
        nohup npx next start -p "$WEB_PORT" >"$WEB_LOG" 2>&1 & )
      wait_http "http://localhost:$WEB_PORT/login" 40 || fail "UI did not come up — see $WEB_LOG"
    fi
  else
  ...
  say "key      $(psql -h 127.0.0.1 -p 5432 -U bettersentryio -d bettersentryio -tAc \
