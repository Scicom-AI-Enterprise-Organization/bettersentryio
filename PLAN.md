...
| D14 | **The official `sentry_sdk` is the error client; bettersentryio speaks Sentry's wire protocol on ingest** (Husein's call, 2026-08-18). The M2-as-built custom JSON ingest (`/api/0/errors` + vendored client error capture) is legacy: kept for zero-dep callers, frozen otherwise. Full plan: [docs/design/sentry-compat.md](docs/design/sentry-compat.md). | This is D3 made binding after the 2026-08-18 parity audit measured the cost of the deviation: the same KeyError captured 45.3 KB by sentry_sdk (locals, ±5 context lines, tags, breadcrumbs, request headers, contexts) vs 8.3 KB by our client. Accepting SDK envelopes buys all of it server-side instead of chasing a mature SDK client-side. | **Growing our own client to parity** — permanent feature chase, and every service already carries sentry_sdk anyway. |
...
> **Status 2026-08-18:** what shipped as "M2 error tracking" deviated from this spec —
> a custom JSON ingest (`/api/0/errors`) + vendored Python client instead of the envelope
> endpoint. Working and deployed, but it fails this section's own "Done when" (an unmodified
> sentry-sdk cannot report to it), and the parity audit measured the capture cost. Being
> refactored back to spec per **D14** — plan and work items in
> [docs/design/sentry-compat.md](docs/design/sentry-compat.md). The "Done when" contract
> above is unchanged and is the acceptance test.
